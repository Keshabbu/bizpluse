...
# Database
# =========================
db_uri = os.environ.get("DATABASE_URL", "sqlite:///bizpulse.db")
...
def auth():
    if request.method == "POST":
        ...
        if action == "register":
            ...
            if existing:
                return render_template("auth.html", message="User exists!", tab="register")
            user = User(username=username, email=email)
            user.set_password(password)
            db.session.add(user)
            db.session.commit()
            return render_template("auth.html", message="Registered successfully!", tab="login")
        if action == "login":
